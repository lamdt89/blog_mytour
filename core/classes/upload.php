...
class upload{
	...
	*/
	...
	function upload($upload_name, $upload_path, $extension_list, $limit_size, $check_image = 1, $prefix="", $zip=0, $url=""){

      // Nếu $upload_name == "" thì return luôn để phục vụ resize image
      if($upload_name == "" && $url == "") return;
      ...
      if ($url == "") {
      ...
      }else {  // neu la upload anh tu url
         //Check upload extension
         if($this->checkExtension($upload_name, $extension_list) != 1){
            $this->common_error     = "&bull; Phần mở rộng của file không đúng, bạn chỉ upload được những file có phần mở rộng là: " . strtoupper($extension_list) . "<br />";
            $this->warning_error    = $this->common_error;
            return;
         }
         ...
         if (!$content = @file_get_contents($url)) {
		      ...
		      $this->common_error     = "&bull; HTTP request failed. Error was: " . $error['message'];
            $this->warning_error    = $this->common_error;
            return;
			}

   //       if (!strpos($http_response_header[0], "200")) {
   //          $this->common_error     = "&bull; Url không tồn tại.<br />";
   //          $this->warning_error    = $this->common_error;
   //          return;
			// }

         $upload = @file_put_contents($upload_path . $upload_name, $content);
         // neu upload that bai
         if ($upload === false) {
            $this->common_error     = "&bull; Có lỗi xảy ra trong quá trình tải file.<br />";
            $this->warning_error    = $this->common_error;
            return;
         }else{	// neu upload thanh cong
            //Check file_size
            if($upload > $limit_size * 1024){
               $this->common_error     = "&bull; Dung lượng file lớn hơn giới hạn cho phép: " . number_format($limit_size, 0, ".", ",") . " KB.<br />";
               $this->warning_error    = $this->common_error;
               return;
            }
            ...
            //Generate new filename
	         $new_filename              = $this->generate_name($upload_name, $prefix);

	         $this->original_name       = $upload_name;
	         $this->file_name           = $new_filename;
	         $this->file_size           = $upload;

	         copy($upload_path . $upload_name, $upload_path . $new_filename);
	         $this->delete_file($upload_path, $upload_name);
