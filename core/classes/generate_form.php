...
class generate_form{
	...
	3). $data_type				: Kieu du lieu , 0 : string , 1 : kieu int, 2 : kieu email, 3 : kieu double, 4 : kieu hash password, 5 : xuống dòng, 6 : không remove HTML tags
	...
	function generate_insert_SQL($multi=0, $replace_into = false, $ignore = false){
		...
		for($i=0; $i<=$this->number_of_field; $i++){
			...
			switch($this->array_data_type[$i]){
				case "0": $str_data .= "'" . strip_tags($temp) . "',"; break;
			...
			if($this->array_data_require[$this->array_data_field[$i]] == 1){
				switch($this->array_data_type[$i]){
					...
					case 6:
						...
						if($temp == "") $errormsg .= "&bull; " . $this->array_data_error_message[$i] . "<br />";
						break;
					case 1:
					...
					case 2:
						...
						$result = preg_match("/^[_a-z0-9-]+(\.[_a-z0-9+-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/i", $temp);
