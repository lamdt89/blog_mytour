...
function checkAccessAddEdit($id_admin, $id_module, $cat_access)
{
   if($id_admin == 1){
      return false;
   }
   $check_add = new db_query("SELECT * FROM admin_user_right WHERE adu_admin_id = " . $id_admin . " AND adu_admin_module_id = " . $id_module);
   $check_add_access = mysql_fetch_assoc($check_add->result);
   if ($check_add_access['adu_'.$cat_access] != 1) {
      return true;
   } else {
      return false;
   }
...
function checkSuperMod($id_admin){
   $check_add = new db_query("SELECT * FROM admin_user WHERE adm_access_module = 1 AND adm_id = ".$id_admin);
   $check_add_access = mysql_fetch_assoc($check_add->result);
   if($check_add_access['adm_access_module'] == 1){
      return true;
   }else{
      return false;
   }
...
function checkAccessCategory(){
	...
	if($row = mysql_fetch_array($db_category->result)){
		...
		if($row["adm_isadmin"] == 1) {
			$db_getall = new db_query("SELECT cat_id FROM categories");			
