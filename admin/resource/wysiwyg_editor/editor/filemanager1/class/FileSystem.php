...
/* PUBLIC METHODS ****************************************************************************** */
	...
	function readDir($dir, $noLog = false) {
		...
		if(is_array($list)) {
			if(!$noLog) $this->addMsg("Read directory $dir");
			return $list;
		}
	...
	function writeFile($path, &$data) {
		...
		if($this->checkFtp()) {
			$srcPath = $this->FileManager->tmpDir . '/' . Tools::basename($path, $this->FileManager->encoding);
			$ok = Tools::saveLocalFile($srcPath, $data, $this->FileManager->encoding);

			if($ok && is_file($srcPath)) {
				return $this->putFile($srcPath, $path);
			}
		}
		else $ok = Tools::saveLocalFile($path, $data, $this->FileManager->encoding);
		$path = $this->checkPath($path);
