<?php 
	...
	if(isset($_SESSION['ses_mem_id'])){
		...
		$post = array(); 
 		$count_post = 6;
 		$first_post = ($page-1)*$count_post;
  		$sql = "SELECT * FROM posts ".$check_list." ORDER BY pos_id DESC LIMIT ".$first_post.",".$count_post;
		...
		$check_list = " INNER JOIN categories ON posts.pos_cat_id = categories.cat_id WHERE cat_show_menu = 1 AND pos_active = 1";
...
<div class="row list-post">
	...
	<div class="col-md-12">
		<div class="row">
			<div class="col-md-3">
				<a href="http://<?=$_SERVER['HTTP_HOST'].'/p'.$item['pos_id'].'-'.$string?>.html"><img width="100%" height="130px" src="uploads/images/posts/<? echo date('Y/m/d/',$item['pos_time']).$item['pos_img'] ?>"></a>
			</div>
